...
#
# This code has been modified by E. van Tegelen, 2025, for research purposes.
# Modifications include the addition of extra attributes and minor refactoring.
# -----------------------------------------------------------------------------
...
class CNO1d(nn.Module):
    ...
    def forward(self, u, x):
        x = x.unsqueeze(-1)                           # [B, X, 1]   
                                                                    
        features = torch.cat([u,  x], dim=-1)         # [B, X, V + P + 1]
        features = features.permute(0, 2, 1)               # [B, V + P + 1, X]

        # Input features into CNO and perform lift        
        ...
        for i in range(self.N_layers):
            ...
            if i == 0:
                h = self.ED_expansion[self.N_layers - i](h) #BottleNeck : no cat
            else:
                h = torch.cat((h, self.ED_expansion[self.N_layers - i](skip[-i])),1)

            # Apply (U) block
            h = self.decoder[i](h)

        # Cat & Execute Projetion
        h = torch.cat((h, self.ED_expansion[0](skip[0])),1)
        h = self.project(h)

        return h.permute(0, 2, 1).unsqueeze(1)  # [B, 1, X, V]
